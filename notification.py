...
def create_notification(pos, color, text, size):
    notification = ET.Element('text', {'extra': 'true', 'name':NOTIFICATION_TEXT_NAME})
    position = ET.SubElement(notification, 'pos').text = pos
    color = ET.SubElement(notification, 'color').text = color
    text = ET.SubElement(notification, 'text').text = text
    size = ET.SubElement(notification, 'size').text = size
    return notification


notification = create_notification('0.5 0.5', '92b1ff', 'UPDATE AVAILABLE', '1 1')

# carbon needs changing in carbon.xml careful comments
notification_carbon = create_notification('0.80 0.935', '8b0000', 'UPDATE AVAILABLE', '0 0')

# all es-theme-epicnoir-*
notification_epicnoir = create_notification('0.05 0.02', 'A3332D', 'UPDATE AVAILABLE', '0 0')

# all CrispRD themes
notification_crisprd = create_notification('0.80 0.02', '92b1ff', 'UPDATE AVAILABLE', '0 0')
